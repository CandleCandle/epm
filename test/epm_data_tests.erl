...
it_creates_a_gen_server_test() ->
	{ok, Pid} = epm_data:start_link(module_name),
...
failure_to_mock_results_in_an_error_test() ->
	{ok, Pid} = epm_data:start_link(module_name),
	?assertEqual({error, {not_stubbed, module_name, func, []}}, gen_server:call(Pid, {call, func, []})).
...
using_the_any_matcher_with_mismatching_arg_lengths_test() ->
	{ok, Pid} = epm_data:start_link(module_name),
	ok = gen_server:call(Pid, {stub, func, [epm:any()], {return, ok}}),
	?assertEqual({error,{not_stubbed,module_name,func,[yeah,that]}}, gen_server:call(Pid, {call, func, [yeah, that]})).
